...
const simpleText = ref(''), simpleNumber = ref(0), simpleRange = ref(50), agreed = ref(false), selectedDay = ref(''), selectedDays = ref([]), working = ref(false), simpleEmail = ref(''), simpleURL = ref(''), simplePswd = ref(''), simpleWords = ref(''), simpleTel = ref('')
function doWork() {
  working.value = true;
  ...
  <main>
    <form @submit.prevent="onSubmit" oninput="sum.value = parseInt(num.value) + parseInt(range.value)">
      ...
      <fieldset>
        ...
        <label>Tel<input type="tel" v-model="simpleTel"></label>
