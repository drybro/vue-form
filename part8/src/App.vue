...
const zipcode=ref(''), pocode=ref('');
function zipdown(e){
  if(e.ctrlKey || e.altKey || e.metaKey || e.key.length>1) return;
  if(/\D/.test(e.key))e.preventDefault();
}
function zipinput(e){
  const str = e.target.value.replace('-','').slice(0,9);
  let place = e.target.selectionEnd;
  e.target.value = str.slice(0,5);
  if(str.length>5) e.target.value += '-' + str.slice(5);
  e.target.setCustomValidity(e.target.validity.patternMismatch?'00000 or 00000-0000':'');
  zipcode.value = e.target.value;
  if(place===6 && str.length===6) place++;
  e.target.selectionEnd = place;
}
function poinput(e){
  const str = e.target.value.replace(/\s+/g,'').slice(0,6).toUpperCase();
  let place = e.target.selectionEnd;
  e.target.value = str.slice(0,3);
  if(str.length>3) e.target.value += ' ' + str.slice(3);
  e.target.setCustomValidity(e.target.validity.patternMismatch?'A0A 0A0':'');
  pocode.value = e.target.value;
  if(place===4 && str.length===4) place++;
  e.target.selectionEnd = place;
}
function podown(e){
  if(e.ctrlKey || e.altKey || e.metaKey || e.key.length>1) return;
  const b = e.target.value.slice(0,e.target.selectionStart).trim().slice(-1);
  if(!/(?!.*[DFIOQU])(^[A-VXY]$|^[a-z][\d]$|^[\d][a-z]$)/i.test(b + e.key))
    e.preventDefault();
  else{
    if(e.target.selectionStart===3) e.target.selectionStart++;
...
<template>
  ...
  <main>
    <form @focusin="$event.target.classList.add('warning')" @submit.prevent="onSubmit"
      oninput="sum.value = parseInt(num.value) + parseInt(range.value)">
      <fieldset><legend>Masks</legend>
        <label>Zip Code<input type="tel" v-model="zipcode" @keydown="zipdown" pattern="^\d{5}(-\d{4})?$" @input="zipinput" placeholder="00000 or 00000-0000" title="5 or 9 digits"></label>
        <label>Post Code<input type="text" v-model="pocode" @keydown="podown" 
          pattern="^(?!.*[DFIOQU])[A-VXY]\d[A-Z] ?\d[A-Z]\d$" @input="poinput" placeholder="A0A 0A0" title="Format A0A 0A0 some letters excluded."></label>
      </fieldset>
      ...
      <fieldset><legend>Text</legend>
        <label class="req">Event<input type="text" list="numbs" :value="simpleText"
            @input="simpleText = $event.target.value" name="word" required placeholder="Required" minlength="5"></label>
        <label>v-model<input type="text" list="daysOfWeek" v-model.lazy="simpleText"></label>
        <label class="req">textArea<textarea v-model.trim.number="simpleWords" name="details" required
            placeholder="Required" maxlength="255"></textarea>
        ...
        <label class="req">Tel<input type="tel" name="phone" required placeholder="Required 555-1234" v-model="simpleTel"
            pattern="(\d\d\d)-(\d\d\d\d)"
            @input="$event.target.setCustomValidity($event.target.validity.patternMismatch ? 'Format 555-1234' : '')"></label>
      ...
      <fieldset><legend>Number</legend>
        <label class="req">Number<input type="number" list="numbs" id="num" v-model="simpleNumber" name="numb" required
            placeholder="Required" min="5" max="100"></label>
      ...
      <fieldset><legend>Select Multiple values from a List</legend>
        <fieldset :class="{ 'groupWarning': areDaysInvalid }">
